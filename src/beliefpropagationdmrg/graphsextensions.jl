...
using NamedGraphs.GraphsExtensions: a_star, add_edge!, dfs_tree, post_order_dfs_edges, add_edge
...
function make_all_degrees_even(g::AbstractGraph)
  ...
  while !isempty(vertices_odd_degree)
    ...
    vertex_pairs = filter(vp -> edgetype(g_modified)(first(vp) => last(vp)) ∉ edges(g_modified), vertex_pairs)
    vertex_pair = first(sort(vertex_pairs; by=vp -> length(a_star(g, vp...))))
    vsrc, vdst = first(vertex_pair), last(vertex_pair)
    e = edgetype(g_modified)((vsrc => vdst))
    g_modified = add_edge(g_modified, e)
    vertices_odd_degree = filter(
      v -> v != vsrc && v != vdst, vertices_odd_degree
    )
  end
  return g_modified
end
...
function original_bp_region_plan(
  g::AbstractGraph,
  start_vertex=default_root_vertex(g);
  nsites::Int=1,
  add_additional_traversal=false,
